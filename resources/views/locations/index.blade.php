                        ...
                        <div class="pm_blog_featured_image_wrapper">
                            <div class="post-short-intro">
                                    ...
                                    @if($map->slug == $real_tag->slug)
                                        @if($real_tag->posts()->where('published', '1')->count() > 0)
                                            <a href="{{ route('tags.show', $map->slug) }}">
                                                <span class='pm_add_icon'><i class='pm_load_more_back fa fa-map-o fa-lg'></i></span> {{ $real_tag->posts()->where('published', '1')->count() }}
                                            </a>
                                        @endif
                                    @endif
                                @endforeach
                                ...
                                <div class="pm_post_likes_wrapper">
                                    <a class="pm_portfolio_read_more" href="location/{{ $map->slug }}"></a>
                                </div>
                            </div>
...
@section('script')
    ...
    @include('components._locations-list')

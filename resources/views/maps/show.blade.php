...
@section('bottom-footer-left-menu')
    <nav class="nav-slit">
        @if($next_map)
            <a class="prev simple-next" href="{{$next_map->id}}">
        ...
        @endif
        @if($previous_map)
            <a class="next simple-next" href="{{$previous_map->id}}">
                        <span class="icon-wrap"><svg class="icon" width="22" height="22" viewBox="0 0 64 64"><use
    ...
    <ul class="nav navbar-nav short-menu">
        ...
        @if($map->published == 0)
            <li rel="tooltip" title="Nepaskelbta">
                <a href="{{ route('maps.show', $map->id, 'Nepaskelbta') }}" aria-label="Nepaskelbtas įrašas">
                    <i class="fa fa-eye-slash fa-lg">
                    </i>
                </a>
            </li>
            <li rel="tooltip" title="Redaguoti">
                <a href="{{ route('maps.edit', $map->id, 'Edit') }}" aria-label="Redaguoti įrašą">
        ...
        @else
            <li rel="tooltip" title="Peržiūra">
                <a href="{{ route('location.slug', $map->slug, 'Preview') }}" aria-label="Peržiūrėti įrašą">
        ...
        @if($map->published == 1 and Auth::check() and Auth::user()->hasRole('editor|administrator|superadministrator'))
            <li rel="tooltip" title="Redaguoti">
                <a href="{{ route('maps.edit', $map->id, 'Edit') }}" aria-label="Redaguoti įrašą">
                    <i class="fa fa-pencil-square fa-lg">
                    </i>
        ...
        @endif
        @if(Auth::check() and Auth::user()->hasPermission('delete-maps'))
            <li rel="tooltip" title="Naikinti" class="post-delete-confirm">
                <a href="javascript:void(0)" aria-label="Naikinti įrašą">
                    <i class="fa fa-trash-o fa-lg"></i>
                </a>
            </li>
            <li rel="tooltip" title="Patvirinti naikinimą" id="submit-button" class="hidden">
